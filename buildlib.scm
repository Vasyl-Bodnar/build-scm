  ...
  (when conditional
    ...
    (let* ((metadata (in-vicinity *obj-build-directory* "metafile"))
      ...
      (when *executable*
        ...
        (check-objs exe-objs)
        (if (and (check-fail)
                 (member (lambda (x) (eq? (car x) 'finished)) exe-objs))
            (link-exe (map caddr exe-objs))
            (info "Already linked, skipping executable"))
        (if (and (check-fail)
                 (member (lambda (x) (eq? (car x) 'finished)) exe-objs))
            (hash-inputs exe-objs)))
      (when (and *library* (memq *library-type* '(static dynamic both)))
        ...
        (if (and (check-fail)
                 (member (lambda (x) (eq? (car x) 'finished)) lib-objs))
            (link-lib (map caddr lib-objs))
            (info "Already linked, skipping library")))
      (if (and (check-fail)
